...
<div class="onlec_mg_list">
	<c:if test="${result.offline_lecture_img eq null}">
		<form id="form" name="form" action="./offlineUpdate?offline_lecture_NO=${result.offline_lecture_NO}" method="post" enctype="multipart/form-data">
	</c:if>
	...
	<button type="button" class="onlec_submit_btn" onclick="checkForm();">수정하기</button>
		...
		<a href="./list" class="onlec_submit_btn"><span>취소</span></a>
	...
	<script>
		...
		function checkForm() {
			...
			if ($("#offline_lecture_img1").val() == '') {
				alert('파일을 선택해주세요');
				$("#offline_lecture_img1").focus();
				return false;
			}

			if ($("#offline_lecture_img1").val() != '' && $("#offline_lecture_img1") > 0) {
				var imgFileExt = $("#offline_lecture_img1").val().split('.').pop().toLowerCase();
