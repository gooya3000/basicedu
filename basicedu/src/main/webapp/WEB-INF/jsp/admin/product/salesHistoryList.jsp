	...
	<tbody>
		...
		<c:forEach var="result" items="${salesHistoryList}" varStatus="status">
			<tr>
				...
				<td>${result.id}</td>
				<td class="txt-center"><fmt:formatDate value="${result.orderDate}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
				<td class="txt-center">
					<c:choose>
				...
				<td class="txt-center">
					<c:if test="${not empty result.contents}">
						<button type="button" class="btn is-red" onclick="delReview('${result.reviewNo}')">삭제</button>
					</c:if>
				</td>
			</tr>
		...
		<c:if test="${fn:length(salesHistoryList) eq 0}">
			<tr>
				<td colspan="6" class="txt-center">판매내역이 없습니다.</td>
			</tr>
		</c:if>
	</tbody>
</table>
